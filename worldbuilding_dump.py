...
def show_statistics(conn: sqlite3.Connection, category: str | None = None) -> None:
    ...
    if result:
        ...
        print(
            f"Total categories: {categories}, Total fields: {fields}, Total content size: {total_size}"
        )
...
Examples:
  ...
  %(prog)s -e worldbuilding.csv   # Export to CSV
    ...
    parser.add_argument(
        "-f", "--full", action="store_true", help="Show full content (default)"
    )
    parser.add_argument(
        "-c", "--category", type=str, help="Show only specific category"
    )
    parser.add_argument("-e", "--export", type=str, help="Export to file (txt or csv)")
    parser.add_argument("-d", "--database", type=str, help="Use specific database file")
    ...
    if not db_path.exists():
        print(
            f"{Colors.RED}Error: Database not found at {db_path}{Colors.NC}",
            file=sys.stderr,
        )
        print(
            "Set STORY_DATABASE_PATH environment variable to specify a different location",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        if args.category:
            ...
            if args.category not in available_categories:
                print(
                    f"{Colors.RED}Error: Invalid category '{args.category}'{Colors.NC}",
                    file=sys.stderr,
                )
                print(
        ...
        if args.export:
            ...
            if export_path.suffix.lower() == ".csv":
                ...
                print(
                    f"{Colors.RED}Error: Unsupported export format. Use .txt or .csv{Colors.NC}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
