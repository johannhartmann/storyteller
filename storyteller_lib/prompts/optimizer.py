...
def summarize_character(
    character_data: dict[str, Any], max_words: int = 100
) -> dict[str, Any]:
...
def truncate_scene_content(
    scene_content: str,
    keep_start: int = 300,
    keep_end: int = 200,
    preserve_dialogue: bool = True,
    preserve_character_moments: bool = True,
) -> str:
    ...
    logger.info(
        f"Returning full scene content ({total_words} words) - truncation disabled"
    )
    return scene_content


def get_relevant_characters(
    all_characters: dict[str, Any], context: str, max_characters: int = 5
) -> dict[str, Any]:
...
def create_context_summary(
    ...
    recent_scenes = (
        previous_scenes[-max_scenes:]
        if len(previous_scenes) > max_scenes
        else previous_scenes
    )
...
def create_character_summary_batch(
    characters: dict[str, Any], max_characters: int = 10
) -> str:
    ...
    if len(characters) > max_characters:
        summary_lines.append(
            f"... and {len(characters) - max_characters} other characters"
        )

    return "\n".join(summary_lines)


def create_plot_thread_summary(
    plot_threads: list[dict[str, Any]], max_threads: int = 5
) -> str:
