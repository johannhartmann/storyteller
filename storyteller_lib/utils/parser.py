...
class StoryElements(BaseModel):
    ...
    characters: list[str] = Field(
        default_factory=list, description="Main character names or descriptions"
    )
    themes: list[str] = Field(
        default_factory=list, description="Key themes or messages"
    )
...
def parse_initial_idea(
    ...
    try:
        ...
        prompt = render_prompt(
            "parse_initial_idea", language=language, initial_idea=initial_idea
        )
