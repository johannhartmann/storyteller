...
def merge_characters(
    existing: CharacterProfileDict, new: CharacterProfileDict
        ...
        if char_id in result:
            ...
            if "evolution" in char_data and char_data["evolution"] is not None:
                ...
                if char_data["evolution"]:
                    if (
                        "evolution" in result_char
                        and result_char["evolution"] is not None
                    ):
                        result_char["evolution"] = (
...
def merge_scenes(existing: SceneStateDict, new: SceneStateDict) -> SceneStateDict:
    ...
    for scene_id, scene_data in new.items():
        if scene_id in result:
            ...
            if (
                "structured_reflection" in scene_data
                and scene_data["structured_reflection"]
            ):
                result_scene["structured_reflection"] = scene_data[
            ...
            if "reflection_notes" in scene_data:
                ...
                if (
                    len(scene_data["reflection_notes"]) == 1
                    and scene_data["reflection_notes"][0] == "Scene has been revised"
                ):
                ...
                # Otherwise append
                    ...
                    if "reflection_notes" in result_scene:
                        result_scene["reflection_notes"] = (
                            result_scene["reflection_notes"]
                            + scene_data["reflection_notes"]
                        )
                    else:
                        result_scene["reflection_notes"] = scene_data[
                            "reflection_notes"
                        ]
...
def merge_chapters(
    existing: ChapterStateDict, new: ChapterStateDict
) -> ChapterStateDict:
        ...
        if chapter_id in result:
            ...
            if "reflection_notes" in chapter_data and chapter_data["reflection_notes"]:
                if "reflection_notes" in result_chapter:
                    result_chapter["reflection_notes"] = (
                        result_chapter["reflection_notes"]
                        + chapter_data["reflection_notes"]
                    )
                else:
                    result_chapter["reflection_notes"] = chapter_data[
                        "reflection_notes"
                    ]
...
def merge_revelations(existing: dict[str, Any], new: dict[str, Any]) -> dict[str, Any]:
    ...
    for issue in old_issues:
        ...
        if chapter not in best_issues_by_chapter:
            ...
            issue.get("resolution_status") == "completed"
            and best_issues_by_chapter[chapter].get("resolution_status") != "completed"
        ):
    ...
    for issue in new_issues:
        ...
        if chapter not in best_issues_by_chapter:
            ...
            issue.get("resolution_status") == "completed"
            or best_issues_by_chapter[chapter].get("resolution_status") != "completed"
        ):
...
def merge_plot_threads(
    ...
    for thread_name, thread_data in new.items():
        if thread_name in result:
            ...
            if (
                "development_history" in thread_data
                and thread_data["development_history"]
            ):
...
def merge_world_elements(
    existing: WorldElementsDict, new: WorldElementsDict
) -> WorldElementsDict:
    """
