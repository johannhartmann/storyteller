...
class CharacterArcPlanFlat(BaseModel):
    ...
    arc_type: str = Field(
        description="Type of arc (positive change, negative change, flat, circular)"
    )
    # Arc stages flattened
    arc_stage_names: str = Field(description="Pipe-separated list of arc stage names")
    arc_stage_descriptions: str = Field(
        description="Pipe-separated list of what happens in each stage"
    )
    arc_stage_emotional_states: str = Field(
        description="Pipe-separated list of emotional states for each stage"
    )
    arc_stage_turning_points: str = Field(
        description="Pipe-separated list of key moments or realizations"
    )
    # Relationship dynamics flattened
    relationship_characters: str = Field(
        description="Pipe-separated list of character names"
    )
    relationship_evolutions: str = Field(
        description="Pipe-separated list of how each relationship evolves"
    )
    thematic_connection: str = Field(description="How the arc reflects story themes")
    emotional_journey_summary: str = Field(
        description="Summary of emotional progression"
    )
...
class CharacterArcPlansFlat(BaseModel):
    ...
    all_arc_stage_names: str = Field(
        description="Double-pipe-separated groups of pipe-separated stage names"
    )
    all_arc_stage_descriptions: str = Field(
        description="Double-pipe-separated groups of pipe-separated descriptions"
    )
    all_arc_stage_emotional_states: str = Field(
        description="Double-pipe-separated groups of pipe-separated emotional states"
    )
    all_arc_stage_turning_points: str = Field(
        description="Double-pipe-separated groups of pipe-separated turning points"
    )
    # All relationship dynamics flattened
    all_relationship_characters: str = Field(
        description="Double-pipe-separated groups of pipe-separated character names"
    )
    all_relationship_evolutions: str = Field(
        description="Double-pipe-separated groups of pipe-separated relationship evolutions"
    )
    thematic_connections: str = Field(
        description="Pipe-separated list of thematic connections"
    )
    emotional_journey_summaries: str = Field(
        description="Pipe-separated list of emotional journey summaries"
    )
...
class CharacterArcStage(BaseModel):
    ...
    emotional_state: str = Field(
        description="Character's emotional state in this stage"
    )
...
class CharacterArcPlan(BaseModel):
    ...
    arc_stages: list[CharacterArcStage] = Field(
        description="Stages of the character arc"
    )
    relationship_dynamics: dict[str, str] = Field(
        description="How relationships evolve"
    )
    thematic_connection: str = Field(description="How the arc reflects story themes")
    emotional_journey_summary: str = Field(
        description="Summary of emotional progression"
...
class CharacterArcPlans(BaseModel):
    ...
    character_arcs: list[CharacterArcPlan] = Field(
        description="List of character arc plans"
    )
...
def plan_character_arcs(
    ...
    for char_id, char_data in characters.items():
        char_info = {
            ...
            "initial_state": char_data.get("emotional_state", {}).get(
                "initial", "Unknown"
            ),
    ...
    try:
        ...
        all_stage_descriptions = [
            g.strip() for g in result.all_arc_stage_descriptions.split("||")
        ]
        all_stage_emotional = [
            g.strip() for g in result.all_arc_stage_emotional_states.split("||")
        ]
        all_stage_turning = [
            g.strip() for g in result.all_arc_stage_turning_points.split("||")
        ]
        all_rel_chars = [
            g.strip() for g in result.all_relationship_characters.split("||")
        ]
        all_rel_evolutions = [
            g.strip() for g in result.all_relationship_evolutions.split("||")
        ]
        thematic_connections = [
            t.strip() for t in result.thematic_connections.split("|")
        ]
        emotional_summaries = [
            s.strip() for s in result.emotional_journey_summaries.split("|")
        ]
        ...
        for i, name in enumerate(names):
            ...
            stages = []
            stage_names = [
                s.strip() for s in all_stage_names[i].split("|") if s.strip()
            ]
            stage_descs = [
                s.strip() for s in all_stage_descriptions[i].split("|") if s.strip()
            ]
            stage_emotions = [
                s.strip() for s in all_stage_emotional[i].split("|") if s.strip()
            ]
            stage_turnings = [
                s.strip() for s in all_stage_turning[i].split("|") if s.strip()
            ]
                ...
                stages.append(
                    {
                        ...
                        "emotional_state": (
                            stage_emotions[j] if j < len(stage_emotions) else ""
                        ),
                        "turning_point": (
                            stage_turnings[j] if j < len(stage_turnings) else ""
                        ),
                    }
                )
            ...
            rel_evolutions = [
                r.strip() for r in all_rel_evolutions[i].split("|") if r.strip()
            ]
            ...
            arc_plans[name] = {
                ...
                "thematic_connection": (
                    thematic_connections[i] if i < len(thematic_connections) else ""
                ),
                "emotional_journey_summary": (
                    emotional_summaries[i] if i < len(emotional_summaries) else ""
                ),
            }
...
class ArcTypeIdentification(BaseModel):
    ...
    arc_type: str = Field(
        description="The identified arc type: redemption, fall, growth, flat, transformation, disillusionment, or education"
    )
    reasoning: str = Field(
        description="Brief explanation for why this arc type fits the character"
    )
    confidence: int = Field(
        ge=1, le=10, description="Confidence level in the identification (1-10)"
    )
...
def define_arc_stages(
    arc_type: str, character_data: dict[str, Any]
) -> list[dict[str, Any]]:
    """
    ...
    arc_stage_patterns = {
        "redemption": [
            {
                "name": "Establishment of flaw",
                "emotional_state": "Denial or justification",
            },
            {"name": "Inciting incident", "emotional_state": "Defensive or resistant"},
            {"name": "Recognition of flaw", "emotional_state": "Guilt or shame"},
            {
                "name": "Struggle with change",
                "emotional_state": "Conflicted or determined",
            },
            {
                "name": "Sacrifice or atonement",
                "emotional_state": "Acceptance or resolve",
            },
        ...
        "fall": [
            ...
            {
                "name": "Temptation introduced",
                "emotional_state": "Curious or conflicted",
            },
            {"name": "First compromise", "emotional_state": "Rationalization or guilt"},
            {
                "name": "Deeper corruption",
                "emotional_state": "Power or pleasure seeking",
            },
        ...
        "growth": [
            {
                ...
                "emotional_state": "Complacent or frustrated",
            },
            {
                "name": "Challenge to worldview",
                "emotional_state": "Confused or resistant",
            },
        ...
        "flat": [
            ...
            {
                "name": "Return to equilibrium",
                "emotional_state": "Unchanged but affecting change",
            },
        ],
        "transformation": [
            {"name": "Original identity", "emotional_state": "Established but limited"},
            {
                ...
                "emotional_state": "Shocked or destabilized",
            },
            {"name": "Identity crisis", "emotional_state": "Confused or searching"},
            {
                "name": "Exploration of new identity",
                "emotional_state": "Experimental or adaptive",
            },
            {"name": "Rebirth", "emotional_state": "Emerging new self"},
            {
                "name": "Integration of new identity",
                "emotional_state": "Transformed and authentic",
            },
        ],
        "disillusionment": [
            ...
            {
                "name": "Confrontation with reality",
                "emotional_state": "Shocked or betrayed",
            },
            {"name": "Cynicism", "emotional_state": "Bitter or angry"},
            {
                "name": "Acceptance of reality",
                "emotional_state": "Resigned or pragmatic",
            },
        ...
        "education": [
            ...
            {
                "name": "Introduction to new information",
                "emotional_state": "Curious or skeptical",
            },
            {
                "name": "Resistance to learning",
                "emotional_state": "Defensive or confused",
            },
            {
                "name": "Gradual acceptance",
                "emotional_state": "Opening up or questioning",
            },
            {
                "name": "Integration of knowledge",
                "emotional_state": "Enlightened or humbled",
            },
            {
                "name": "Application of wisdom",
                "emotional_state": "Confident or thoughtful",
            },
        ],
    }
    ...
    if (
        "character_arc" in character_data
        and "stages" in character_data["character_arc"]
    ):
        existing_stages = character_data["character_arc"]["stages"]
        if existing_stages and isinstance(existing_stages, list):
            return [
                {"name": stage, "emotional_state": "Varies"}
...
def update_character_arc(
    character: dict[str, Any], scene_content: str, chapter_num: str, scene_num: str
) -> dict[str, Any]:
    """
    ...
    if "character_arc" not in character:
        ...
        character["character_arc"] = {
            "type": arc_type,
            "stages": [
                stage["name"] for stage in define_arc_stages(arc_type, character)
            ],
            "current_stage": "",
        }
    ...
    try:
        ...
        class InnerConflictUpdate(BaseModel):
            ...
            conflict_index: int = Field(
                default=0, description="Index of the conflict to update, or -1 for new"
            )
            ...
            impact: str = Field(
                default="", description="How this affects the character"
            )
        ...
        class CharacterArcUpdate(BaseModel):
            ...
            new_arc_stage: str | None = Field(
                default="", description="The new character arc stage after this scene"
            )
            emotional_state_update: str | None = Field(
                default="", description="The updated emotional state of the character"
            )
            inner_conflict_updates: list[InnerConflictUpdate] = Field(
                default_factory=list,
                description="Updates to the character's inner conflicts",
            )
            emotional_journey_addition: str | None = Field(
                default="",
        ...
        if updates_dict["new_arc_stage"]:
            character_updates["character_arc"] = character.get(
                "character_arc", {}
            ).copy()
            character_updates["character_arc"]["current_stage"] = updates_dict[
                "new_arc_stage"
            ]

        # Update emotional state if provided
        if updates_dict["emotional_state_update"]:
            character_updates["emotional_state"] = character.get(
                "emotional_state", {}
            ).copy()
            character_updates["emotional_state"]["current"] = updates_dict[
        ...
        if updates_dict["emotional_journey_addition"]:
            ...
            journey.append(
                f"Ch{chapter_num}-Sc{scene_num}: {updates_dict['emotional_journey_addition']}"
            )
            character_updates["emotional_state"]["journey"] = journey

        # Update inner conflicts if provided
        if updates_dict.get("inner_conflict_updates") and isinstance(
            updates_dict["inner_conflict_updates"], list
        ):
                ...
                if conflict_update.get("conflict_index", -1) == -1:
                    ...
                    inner_conflicts.append(
                        {
                            ...
                            "resolution_status": conflict_update.get(
                    ...
                    if 0 <= index < len(inner_conflicts):
                        if "description" in conflict_update:
                            inner_conflicts[index]["description"] = conflict_update[
                                "description"
                            ]
                        if "resolution_status" in conflict_update:
                            inner_conflicts[index]["resolution_status"] = (
                                conflict_update["resolution_status"]
                            )
