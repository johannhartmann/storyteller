...
class SceneTypeDistribution:
    ...
    def validate(self):
        ...
        assert (
            abs(total - 1.0) < 0.01
        ), f"Scene type distribution must sum to 1.0, got {total}"
...
class NarrativeStructureAnalysis(BaseModel):
    ...
    chapter_count: int = Field(
        description="Recommended total number of chapters", ge=5, le=50
    )
    scenes_per_chapter: int = Field(
        description="Recommended average scenes per chapter", ge=2, le=10
    )
    words_per_scene: int = Field(
        description="Recommended target words per scene", ge=300, le=2000
    )
    subplot_count: int = Field(description="Estimated number of subplots", ge=0, le=10)
    pov_count: int = Field(
        description="Estimated number of POV characters", ge=1, le=10
    )
