...
class VoiceManager:
    ...
    VOICE_RECOMMENDATIONS = {
        "english": {
            ...
            "dramatic": {"male": ["en-US-DavisNeural"], "female": ["en-US-SaraNeural"]},
        },
        "german": {
            "narrator": {
                "male": [
    ...
    def get_voice_for_language(
        cls, language: str, style: str = "narrator", gender: str = "neutral"
    ) -> str:
        ...
        style_voices = cls.VOICE_RECOMMENDATIONS[language].get(
            style, cls.VOICE_RECOMMENDATIONS[language]["narrator"]
        )
...
class BatchSynthesizer:
    ...
    def __init__(self, speech_key: str, speech_region: str, output_format: str = "mp3"):
        """
        ...
        for idx, item in enumerate(ssml_items):
            try:
                ...
                if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                    ...
                    logger.info(
                        f"Synthesized: {item['filename']} ({file_size / 1024:.1f} KB)"
                    )
                else:
                ...
                logger.error(
                    f"Error synthesizing {item.get('filename', 'unknown')}: {str(e)}"
                )
                results["failed"].append(
                    {
                        "id": item.get("id"),
...
class SSMLEnhancer:
    ...
    @staticmethod
    def add_chapter_announcement(
        chapter_num: int, chapter_title: str, voice_name: str, language: str = "en-US"
    ) -> str:
