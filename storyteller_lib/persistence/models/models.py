...
class StoryDatabase:
    ...
    def _apply_schema_updates(self, conn: sqlite3.Connection) -> None:
        ...
        # Check if instructions column exists in scenes table
        if "instructions" not in scene_columns:
            logger.info("Adding instructions column to scenes table")
        ...
        if "narrative_structure" not in config_columns:
            ...
            cursor.execute(
                "ALTER TABLE story_config ADD COLUMN structure_metadata TEXT"
            )
            cursor.execute(
        ...
        # Check if book_level_instructions column exists separately (in case it's missing)
        if "book_level_instructions" not in config_columns:
            logger.info("Adding book_level_instructions column to story_config table")
