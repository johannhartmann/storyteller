...
class CharacterKnowledgeManager:
    ...
    def add_knowledge(
        ...
        try:
            with self.db_manager._db._get_connection() as conn:
                ...
                if cursor.fetchone():
                    logger.warning(f"Knowledge already exists for character {character_id}: {knowledge}")
                    return False
                ...
                logger.info(f"Added {visibility} knowledge for character {character_id}: {knowledge}")
                return True
    ...
    def reveal_secret(
        ...
        try:
            with self.db_manager._db._get_connection() as conn:
                ...
                if not secret:
                    logger.warning(f"Secret not found for character {character_id}: {secret_content}")
                    return False
                ...
                conn.commit()
                logger.info(f"Revealed secret for character {character_id}: {secret_content}")
                return True
    ...
    def get_character_secrets(
        ...
        for vis in visibility_filter:
            secrets.extend(self.get_character_knowledge(
                character_id,
                visibility=vis,
