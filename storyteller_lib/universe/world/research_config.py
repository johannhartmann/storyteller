...
class WorldBuildingResearchConfig(BaseModel):
    ...
    research_depth: Literal["shallow", "medium", "deep"] = Field(
        default="medium", description="How extensive the research should be"
    )

    queries_per_category: int = Field(
        default=3, description="Number of search queries per world building category"
    )

    max_search_iterations: int = Field(
        default=2, description="Maximum refinement iterations for search"
    )

    include_sources: bool = Field(
        default=True, description="Include source citations in world elements"
    )

    parallel_research: bool = Field(
        default=True, description="Execute category research in parallel"
    )

    language: str = Field(
        default="english", description="Language for prompts and templates"
    )
    ...
    tavily_cache_enabled: bool = Field(
        default=True, description="Enable caching of Tavily API calls"
    )
    ...
    def from_config(cls, config: dict[str, Any]) -> "WorldBuildingResearchConfig":
        ...
        if "tavily_cache_enabled" not in research_config:
            research_config["tavily_cache_enabled"] = (
                os.getenv("TAVILY_CACHE_ENABLED", "true").lower() == "true"
            )

        if "tavily_cache_ttl_days" not in research_config:
            research_config["tavily_cache_ttl_days"] = int(
                os.getenv("TAVILY_CACHE_TTL_DAYS", "30")
            )
