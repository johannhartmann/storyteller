...
class SearchResult(BaseModel):
    ...
    relevance_score: float = Field(
        default=0.0,
        description="Relevance score for this result (0-1)"
    )
    source_type: Optional[str] = Field(
        default="web",
        description="Type of source (web, academic, book, etc.)"
    )
...
class Citation(BaseModel):
    ...
    relevance_score: float = Field(
        default=0.0,
        description="How relevant this citation is (0-1)"
    )
...
class ResearchInsight(BaseModel):
    ...
    supporting_sources: List[Citation] = Field(
        ...
        description="Citations supporting this insight"
    )
...
class ResearchResults(BaseModel):
    ...
    synthesized_insights: Dict[str, str] = Field(
        ...
        description="Key insights synthesized from research"
    )
    relevant_examples: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Concrete examples found in research"
    )
    source_citations: List[Citation] = Field(
        default_factory=list,
        description="All citations for transparency"
    )
    research_summary: Optional[str] = Field(
        default=None,
        description="Overall summary of research findings"
    )
    confidence_score: float = Field(
        default=0.0,
        description="Overall confidence in research quality (0-1)"
    )
...
class CategoryResearchStrategy(BaseModel):
    ...
    focus_areas: List[str] = Field(
        description="Key areas to focus research on"
    )
    query_templates: List[str] = Field(
        description="Template queries for this category"
    )
    ...
    min_sources: int = Field(
        default=3,
        description="Minimum number of quality sources needed"
    )
...
class ResearchContext(BaseModel):
    ...
    story_outline: Optional[str] = Field(
        default=None,
        description="Story outline if available"
    )
    ...
    cultural_context: Optional[str] = Field(
        default=None,
        description="Specific cultural context to consider"
    )
    time_period: Optional[str] = Field(
        default=None,
        description="Time period for historical stories"
    )