...
class WorldBuildingResearcher:
    ...
    async def research_initial_context(
        ...
        search_results = await execute_parallel_searches(
            ...
            max_results_per_query=self.config.get_depth_params()[
    ...
    async def _synthesize_category_insights(
        ...
        if not results:
            return {
                f"{category}_general": f"No specific research findings for {category}"
            }
        ...
        try:
            ...
            language = (
                ...
                else context.get("language", "english")
            )
            ...
            try:
                ...
                logger.warning(
                    f"Category-specific template not found, using generic: {category_error}"
                )
                prompt = render_prompt(
                    "research_synthesis",
                    language,
    ...
    async def _create_research_summary(
        ...
        examples_text = "\n".join(
            [f"- {e['title']}: {e['description']}" for e in examples[:5]]
        )
    ...
    def _calculate_confidence(
        ...
        # Weighted average
        confidence = (
            num_results_score * 0.3 + avg_relevance * 0.5 + citation_score * 0.2
        )
