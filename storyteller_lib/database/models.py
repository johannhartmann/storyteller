...
class StoryDatabase:
    ...
    # Story Management
    def initialize_story_config(self, title: str, genre: str, tone: str, **kwargs) -> None:
        """
        ...
        with self._get_connection() as conn:
            ...
            if exists:
                ...
                query = f"UPDATE story_config SET {', '.join(update_fields)} WHERE id = 1"
                ...
                query = f"INSERT INTO story_config ({field_names}) VALUES ({placeholders})"
    ...
    def update_character_state(self, character_id: int, scene_id: int, state: Dict[str, Any]) -> None:
        """
        ...
        with self._get_connection() as conn:
            ...
            conn.commit()
            logger.debug(f"Updated character state for character {character_id} in scene {scene_id}")
    
    def get_character_state_at_scene(self, character_id: int, scene_id: int) -> Dict[str, Any]:
        """
    ...
    def add_character_knowledge(self, character_id: int, scene_id: int, knowledge: Dict[str, Any]) -> None:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (character_id, scene_id, knowledge.get('knowledge_type'),
                 knowledge.get('knowledge_content'), knowledge.get('source'))
            )
            conn.commit()
            logger.debug(f"Added knowledge for character {character_id} in scene {scene_id}")
    
    # Relationship Management
    def create_relationship(self, char1_id: int, char2_id: int, rel_type: str, **kwargs) -> None:
        """
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (char1_id, char2_id, rel_type,
                 kwargs.get('description'), properties)
            )
            conn.commit()
            logger.debug(f"Created relationship between characters {char1_id} and {char2_id}")
    ...
    def create_world_element(self, category: str, element_key: str,
                           element_value: Union[str, Dict, List]) -> None:
        """
    ...
    def get_world_elements(self, category: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            ...
            if category:
                cursor.execute(
                    "SELECT * FROM world_elements WHERE category = ?",
                    (category,)
                )
            else:
                cursor.execute(
    ...
    def add_plot_thread_development(self, plot_thread_id: int, scene_id: int,
                                   development_type: str, description: str) -> None:
        """
        ...
        with self._get_connection() as conn:
            ...
            conn.commit()
            logger.debug(f"Added plot thread development for thread {plot_thread_id} in scene {scene_id}")
    ...
    def get_scenes_with_entity(self, entity_type: str, entity_id: int) -> List[Dict[str, Any]]:
        """
    ...
    def get_entity_evolution(self, entity_type: str, entity_id: int) -> List[Dict[str, Any]]:
        """
    ...
    def get_scene_context(self, scene_id: int) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            ...
            for char in context['entities']['characters']:
                char_id = char['id']
                context['character_states'][char_id] = self.get_character_state_at_scene(char_id, scene_id)
                context['character_relationships'][char_id] = self.get_character_relationships(char_id)
    ...
    def get_chapter_end_context(self, chapter_id: int) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            ...
            return {
                'chapter_info': dict(chapter) if chapter else {},
                'no_scenes': True
            }
    
    def log_entity_change(self, entity_type: str, entity_id: int, scene_id: int,
        ...
        with self._get_connection() as conn:
            ...
            conn.commit()
            logger.debug(f"Logged {change_type} change for {entity_type} {entity_id} in scene {scene_id}")
    
    # Content Registry Methods
    def register_content(self, content_type: str, content_text: str, 
                        chapter_id: Optional[int] = None, scene_id: Optional[int] = None) -> bool:
        """
    ...
    def get_used_content(self, content_type: Optional[str] = None, 
...
class DatabaseStateAdapter:
        ...
        if 'world_elements' in state and state['world_elements']:
            for category, elements in state['world_elements'].items():
                for key, value in elements.items():
                    self.db.create_world_element(story_id, category, key, value)
        ...
        with self.db._get_connection() as conn:
            ...
            for row in cursor.fetchall():
                ...
                for rel in relationships:
                    char_profile['relationships'][rel['other_character_identifier']] = rel['relationship_type']
    ...
    def update_scene_entities(self, state: StoryState, scene_id: int) -> None:
        ...
        if current_chapter and current_scene:
            scene_content = state['chapters'][current_chapter]['scenes'][current_scene].get('content', '')
...
class StoryQueries:
    ...
    def find_chapters_affected_by_character_change(self, character_id: int,
    ...
    def get_relationship_dynamics_over_time(self, char1_id: int, char2_id: int) -> List[Dict[str, Any]]:
        """
