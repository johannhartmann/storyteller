...
from storyteller_lib.summary_generation import generate_scene_summary, generate_chapter_summary
...
def generate_summaries(state: StoryState) -> Dict:
    ...
    logger.info(f"Generating summaries for Chapter {current_chapter}, Scene {current_scene}")
    ...
    if not scene_content:
        logger.warning(f"No scene content found for Chapter {current_chapter}, Scene {current_scene}")
        return {}
    
    # Generate scene summary
    scene_summary = generate_scene_summary(current_chapter, current_scene, scene_content, language)
    ...
    if is_last_scene:
        logger.info(f"Last scene of Chapter {current_chapter} - generating chapter summary")
