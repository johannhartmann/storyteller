...
class SSMLConverter:
    ...
    def __init__(self, model_provider: str = None, model: str = None, language: str = "english"):
        """
        ...
        self.env = Environment(
            loader=FileSystemLoader([template_path, os.path.join(template_dir, 'base')]),
            autoescape=select_autoescape(['html', 'xml'])
        )
        
    def scene_to_ssml(self, scene_content: str, chapter_number: int, 
                      scene_number: int, scene_description: str,
                      genre: str, tone: str, chapter_title: str = None,
                      book_title: str = None, is_first_scene_in_chapter: bool = False,
                      is_first_scene_in_book: bool = False) -> str:
        """
        ...
        try:
            ...
            if not ssml_content.startswith('<speak'):
                ...
                if 'xml:lang' not in ssml_content:
                    ...
                    ssml_content = ssml_content.replace('<speak>', f'<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="{self._get_lang_code()}">')
    ...
    def convert_book_to_ssml(self, db_path: str, output_path: str) -> None:
        ...
        try:
            ...
            ssml_parts = []
            ssml_parts.append(f'<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="{self._get_lang_code()}">')
            ...
            with db._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if row['chapter_number'] != current_chapter:
                        ...
                        ssml_parts.append(f'<emphasis level="moderate">Chapter {current_chapter}: {row["chapter_title"]}</emphasis>')
                        ssml_parts.append('<break time="1500ms"/>')
                    
                    # Update progress
                    scene_count += 1
                    print(f"Converting scene {scene_count}/{total_scenes}: Chapter {row['chapter_number']}, Scene {row['scene_number']}...", end='', flush=True)
                    
                    # Determine if this is the first scene in chapter/book
                    is_first_scene_in_chapter = row['scene_number'] == 1
                    is_first_scene_in_book = row['chapter_number'] == 1 and row['scene_number'] == 1
