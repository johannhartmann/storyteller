...
def identify_key_concepts(
    global_story: str, genre: str, language: str = DEFAULT_LANGUAGE
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        class KeyConcept(BaseModel):
            ...
            introduced: bool = Field(
                default=False, description="Whether the concept has been introduced"
            )
            introduction_chapter: str = Field(
                default="", description="Chapter where concept was introduced"
            )
            introduction_scene: str = Field(
                default="", description="Scene where concept was introduced"
            )
        ...
        class KeyConceptsAnalysis(BaseModel):
            ...
            key_concepts: list[KeyConcept] = Field(
                default_factory=list, description="List of key concepts"
            )
...
def check_concept_introduction(state: dict) -> dict[str, Any]:
    ...
    try:
        results = search_memory(
            query="key_concepts_tracker", namespace=MEMORY_NAMESPACE
        )
        ...
        for concept in key_concepts_tracker["key_concepts"]:
            if (
                concept["recommended_chapter"] == current_chapter
                and not concept["introduced"]
            ):
        ...
        return {"concepts_to_introduce": concepts_for_current_chapter}
...
def update_concept_introduction_status(state: dict, concept_name: str) -> dict:
    ...
    try:
        ...
        updated_tracker = {"key_concepts": updated_concepts}
        ...
        return {"key_concepts_tracker": updated_tracker}
...
def analyze_concept_clarity(
    scene_content: str, concept_name: str, language: str = DEFAULT_LANGUAGE
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        class ConceptClarity(BaseModel):
            ...
            clarity_score: int = Field(
                ge=1, le=10, description="Clarity score (1=unclear, 10=perfectly clear)"
            )
            ...
            strengths: list[str] = Field(
                default_factory=list, description="Strengths of the exposition"
            )
            weaknesses: list[str] = Field(
                default_factory=list, description="Weaknesses of the exposition"
            )
...
def generate_exposition_guidance(
    concepts_to_introduce: list[dict[str, Any]],
    genre: str,
    tone: str,
...
def check_and_generate_exposition_guidance(state: dict) -> dict:
    ...
    exposition_guidance = generate_exposition_guidance(
        concepts_to_introduce, genre, tone, language
    )

    # Add sensory checklists for each concept
    sensory_checklists = {}
    for concept in concepts_to_introduce:
        sensory_checklists[concept["name"]] = generate_concept_sensory_checklist(
            concept, language
        )
...
def analyze_showing_vs_telling(
    scene_content: str,
    genre: str = "fantasy",
    ...
    try:
        ...
        class TellingInstance(BaseModel):
            ...
            text: str = Field(description="The text that tells rather than shows")
            issue: str = Field(description="What type of telling issue this is")
        ...
        class ShowingInstance(BaseModel):
            ...
            text: str = Field(description="The text that effectively shows")
            strength: str = Field(
        ...
        class ShowingTellingAnalysis(BaseModel):
            ...
            emotion_showing_score: int = Field(
                ge=1, le=10, description="How well emotions are shown rather than told"
            )
            ...
            showing_instances: list[ShowingInstance] = Field(
                default_factory=list, description="Examples of effective showing"
            )
        ...
        if showing_telling_analysis is None:
            ...
            return {
                ...
                "missed_opportunities": [],
                "improvement_suggestions": [
                    "Error analyzing showing vs. telling: LLM returned None"
                ],
            }
            ...
            try:
                ...
                return {
                    **simple_analysis.dict(),
                    ...
                    "missed_opportunities": [],
                    "improvement_suggestions": [
                        "Error processing detailed analysis, showing basic scores only"
                    ],
                }
...
def generate_concept_sensory_checklist(
    concept: dict[str, Any], language: str = DEFAULT_LANGUAGE
) -> str:
    """
