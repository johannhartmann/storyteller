...
def analyze_dialogue(
    scene_content: str, characters: dict[str, Any], language: str = DEFAULT_LANGUAGE
) -> dict[str, Any]:
    """
    ...
    character_names = []
    for _char_id, char_data in characters.items():
    ...
    try:
        ...
        class DialogueExchange(BaseModel):
            ...
            content: str = Field(description="The content of the dialogue exchange")
            purpose: str = Field(
        ...
        class DialogueAnalysis(BaseModel):
            ...
            naturalness_score: int = Field(
                ge=1, le=10, description="How natural the dialogue sounds"
            )
            character_voice_score: int = Field(
                ge=1, le=10, description="How distinctive each character's voice is"
            )
            exposition_score: int = Field(
                ge=1, le=10, description="How naturally information is revealed"
            )
            subtext_score: int = Field(
                ge=1, le=10, description="How well dialogue uses subtext"
            )
            purpose_score: int = Field(
                ge=1, le=10, description="How purposeful each exchange is"
            )
            efficiency_score: int = Field(
                ge=1, le=10, description="How concise and efficient the dialogue is"
            )
            overall_score: int = Field(
                ge=1, le=10, description="Overall dialogue quality"
            )
            issues: list[DialogueIssue] = Field(
                default_factory=list, description="List of specific dialogue issues"
            )
            strengths: list[str] = Field(
                default_factory=list, description="Dialogue strengths"
            )
...
def _generate_character_dialogue_patterns(
    characters: dict[str, Any], language: str = DEFAULT_LANGUAGE
) -> str:
    """
...
def improve_dialogue(
    ...
    focus_on_exposition: bool = False,
...
def generate_dialogue_guidance(
    characters: dict[str, Any], genre: str, tone: str, language: str = DEFAULT_LANGUAGE
) -> str:
    """
...
def analyze_and_improve_dialogue(state: dict) -> dict:
    ...
    if not db_manager or not db_manager._db:
        raise RuntimeError(
            "Database manager not available - cannot retrieve scene content"
        )
    ...
    if dialogue_analysis["overall_score"] < 8:
        improved_scene = improve_dialogue(
            scene_content, dialogue_analysis, characters, language=language
        )
