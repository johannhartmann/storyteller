...
def analyze_scene_pacing(
    scene_content: str, genre: str, tone: str, language: str = DEFAULT_LANGUAGE
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        class PacingIssue(BaseModel):
            ...
            suggested_revision: str = Field(description="A more concise revision")
            reason: str = Field(description="Why this change improves pacing")
            location: str = Field(description="Beginning, middle, or end of scene")
        ...
        class PacingAnalysis(BaseModel):
            ...
            dialogue_efficiency_score: int = Field(
                ge=1, le=10, description="How concise and effective the dialogue is"
            )
            description_efficiency_score: int = Field(
                ge=1, le=10, description="How efficient the descriptions are"
            )
            ...
            issues: list[PacingIssue] = Field(
                default_factory=list, description="List of specific pacing issues"
            )
...
def analyze_and_optimize_scene(state: StoryState) -> dict:
    ...
    if pacing_analysis["overall_pacing_score"] < 8:
        optimized_scene = optimize_scene_pacing(
            scene_content, pacing_analysis, genre, tone, language
        )

        # Update the scene content in database
        db_manager.save_scene_content(
            int(current_chapter), int(current_scene), optimized_scene
        )
