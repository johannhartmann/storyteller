...
def progress_callback(node_name: str, state: Dict[str, Any]) -> None:
    ...
    progress_manager.state.node_counts[node_name] = (
        progress_manager.state.node_counts.get(node_name, 0) + 1
    )
    ...
    if node_name == "initialize_state":
        ...
        if initial_idea:
            ...
            if "initial_idea_elements" in state:
                ...
                sys.stdout.write(
                    f"- Characters: {', '.join(elements.get('characters', []))}\n"
                )
                sys.stdout.write(f"- Plot: {elements.get('plot', 'Unknown')}\n")
                if elements.get("themes"):
                    sys.stdout.write(
                        f"- Themes: {', '.join(elements.get('themes', []))}\n"
                    )
            sys.stdout.write("-------------------------------\n\n")
        ...
        if "creative_elements" in state:
            ...
            sys.stdout.write("\n------ CREATIVE CONCEPTS ------\n")
            if (
                "story_concepts" in creative
                and "recommended_ideas" in creative["story_concepts"]
            ):
            ...
            if (
                ...
                and "recommended_ideas" in creative["world_building"]
            ):
        ...
        if "global_story" in state:
            ...
            sys.stdout.write(
                f"[Story outline continues with {section_count} more sections...]\n"
            )
            sys.stdout.write("----------------------------------\n\n")
        ...
        if "world_elements" in state:
            ...
            for category, elements in world_elements.items():
                if (
                    category != "mystery_elements" and elements
                ):  # Skip mystery elements for now
            ...
            if (
                "mystery_elements" in world_elements
                ...
                if "red_herrings" in mystery and mystery["red_herrings"]:
                    sys.stdout.write(
                        f"  • Red Herrings: {len(mystery['red_herrings'])}\n"
                    )
        ...
        if "characters" in state:
            ...
            for char_name, char_data in characters.items():
                ...
                if "protagonist" in role or "main" in role or "detective" in role:
                    main_chars.insert(
                        0, (char_name, char_data)
                    )  # Add protagonist first
            ...
            sys.stdout.write(
                f"[Plus {len(characters) - len(main_chars)} additional characters...]\n"
            )
            sys.stdout.write("----------------------------\n\n")
            sys.stdout.write("-------------------------------\n\n")
        ...
        if "chapters" in state:
            ...
            total_scenes = sum(
                len(chapter.get("scenes", {})) for chapter in chapters.values()
            )
            sys.stdout.write(
                f"Story structure: {len(chapters)} chapters with approximately {total_scenes} scenes\n"
            )
            sys.stdout.write(
                f"Estimated final length: {total_scenes * 1500}-{total_scenes * 2500} words\n\n"
            )

            # Show chapter breakdown
            for ch_num in sorted(
                chapters.keys(), key=lambda x: int(x) if x.isdigit() else float("inf")
            ):
                chapter = chapters[ch_num]
                sys.stdout.write(
                    f"CHAPTER {ch_num}: {chapter.get('title', 'Untitled')}\n"
                )
        ...
        if current_chapter in chapters:
            ...
            if (
                current_chapter in chapters
                and current_scene in chapters[current_chapter]["scenes"]
            ):
                scene_content = chapters[current_chapter]["scenes"][current_scene].get(
                    "content", ""
                )
                if scene_content:
                    # Get scene summary
                    ...
                    if "summary" in chapters[current_chapter]["scenes"][current_scene]:
                        scene_summary = chapters[current_chapter]["scenes"][
                            current_scene
                        ]["summary"]
                    ...
                    for para in paragraphs:
                        if '"' in para or "'" in para:
                            dialogue_start = (
                                para.find('"') if '"' in para else para.find("'")
                            )
                            if dialogue_start >= 0:
                                dialogue_end = (
                                    para.find('"', dialogue_start + 1)
                                    if '"' in para
                                    else para.find("'", dialogue_start + 1)
                                )
                    ...
                    sys.stdout.write(
                        f"Scene length: {len(scene_content)} characters, {len(paragraphs)} paragraphs\n"
                    )
                    sys.stdout.write("---------------------------\n\n")
        ...
        if (
            current_chapter in state.get("chapters", {})
            and current_scene in state["chapters"][current_chapter]["scenes"]
        ):
            reflection_notes = state["chapters"][current_chapter]["scenes"][
                current_scene
            ...
            if reflection_notes:
                ...
                sys.stdout.write(
                    f"Analysis of scene {current_scene} in chapter {current_chapter}:\n"
                )
                ...
                if len(reflection_notes) > 3:
                    sys.stdout.write(
                        f"[Plus {len(reflection_notes) - 3} additional notes...]\n"
                    )
        ...
        if progress_manager and progress_manager.state.verbose_mode:
            ...
            if (
                ...
                if (
                    scene.get("reflection_notes")
                    and len(scene["reflection_notes"]) == 1
                    and scene["reflection_notes"][0] == "Scene has been revised"
                ):
                    scene_revised = True
            ...
            if scene_revised:
                ...
                sys.stdout.write(
                    "The scene was revised based on reflection notes and continuity checks.\n"
                )
                sys.stdout.write("--------------------------\n\n")
            else:
                sys.stdout.write(f"\n------ NO REVISION NEEDED [{scene_info}] ------\n")
                sys.stdout.write(
                    "The scene was reviewed and found to be consistent and well-crafted.\n"
                )
                sys.stdout.write("-------------------------------\n\n")
        ...
        if (
            progress_manager
            and progress_manager.state.verbose_mode
            and "revelations" in state
            and "continuity_issues" in state["revelations"]
        ):
            ...
            for issue in continuity_issues:
                sys.stdout.write(
                    f"After Chapter {issue.get('after_chapter', 'unknown')}:\n"
                )
        ...
        scene_count = sum(
            ...
            for chapter in state.get("chapters", {}).values()
        )
    ...
    if node_name == "revise_scene_if_needed":
        try:
            ...
            if (
                ...
                and progress_manager.state.output_file
            ):
                ...
                write_scene_to_file(
                    int(current_chapter),
                    int(current_scene),
                    progress_manager.state.output_file,
                )
                sys.stdout.flush()
        except Exception as e:
            sys.stdout.write(
    ...
    if (
        new_chapter
        and new_scene
        and (current_chapter != new_chapter or current_scene != new_scene)
    ):
            ...
            if new_chapter in chapters:
                ...
                for ch_num, ch_data in chapters.items():
                    ...
                    if scenes_completed:
                        ...
                        if (
                            current_chapter
                            and current_chapter.strip()
                            and int(ch_num) < int(current_chapter)
                        ):
                ...
                progress_pct = (
                    (completed_scenes / total_planned_scenes * 100)
                    if total_planned_scenes > 0
                    else 0
                )
                sys.stdout.write(
                    f"[{elapsed_str}] Overall progress: {progress_pct:.1f}% - {completed_chapters}/{total_chapters} chapters, {completed_scenes}/{total_planned_scenes} scenes\n"
                )
                sys.stdout.flush()
